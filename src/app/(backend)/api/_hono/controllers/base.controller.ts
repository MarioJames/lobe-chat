...
export abstract class BaseController {
   ...
   */
  protected validateParam(value: any, name: string, required: boolean = true): void {
    if (required && (value === undefined || value === null || value === '')) {
      throw new HTTPException(400, { message: `参数 ${name} 不能为空` });
    }
  }
  ...
  protected validateArray(value: any[], name: string, minLength: number = 1): void {
    if (!Array.isArray(value)) {
      throw new HTTPException(400, { message: `参数 ${name} 必须是数组` });
    }

    if (value.length < minLength) {
      throw new HTTPException(400, { message: `参数 ${name} 长度不能少于 ${minLength}` });
    }
  }
}
