...
/**
 * Call external Admin service for sensitive word detection (server-side only).
 */
export const checkSensitiveText = async (params: AdminSensitiveCheckInput) => {
  if (typeof window !== 'undefined')
    throw new Error('checkSensitiveText can only be called on the server side');
  if (!adminEnv.ADMIN_TRPC_URL) throw new Error('ADMIN_TRPC_URL is not configured');
